...
VALUES ('gp-joule', 'app', 'v1.0.0')
	ON CONFLICT (app_name) DO UPDATE SET version = 'v1.0.0';
...
WHERE asset_id IN (
	SELECT asset_id
	FROM asset
	WHERE asset_type LIKE E'gp\\_joule\\_%'
);
...
WHERE widget_id IN (
	...
	WHERE public.dashboard.name LIKE 'GP Joule%'
);
